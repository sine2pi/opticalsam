...
class Sam3ImageOnVideoMultiGPU(Sam3Image):
    def __init__(
        ...
        if gather_backbone_out is None:
            gather_backbone_out = isinstance(self.backbone, SAM3VLBackbone) or self.backbone.__class__.__name__ == "SAM3VLBackbone"
    ...
    def forward_video_grounding_multigpu(
        ...
        **kwargs,
        ...
        out = {}
        return_sam2_backbone_feats = return_sam2_backbone_feats or return_tracker_backbone_feats
        for k, (v, handle) in multigpu_buffer[frame_idx].items():
            if (k.startswith("sam2_backbone_") or k.startswith("tracker_backbone_")) and not return_sam2_backbone_feats:
                continue
