...
class Sam3VideoInference(Sam3VideoBase):
    ...
    def init_state(
        ...
        inference_state = {}
        inference_state["enable_cache"] = enable_cache
    ...
    def _construct_initial_input_batch(
        ...
        if offload_video_to_cpu:
            input_batch.find_inputs = [
                copy_data_to_device(s, device, non_blocking=True) for s in stages
            ]
        else:
    ...
    def _run_single_frame_inference(self, inference_state, frame_idx, reverse):
        ...
        if self.rank == 0 and inference_state.get("enable_cache", True):
    ...
    def add_new_mask(
        ...
        """Add mask direct to tracker."""
        logger.debug("Running add_new_mask on frame %d", frame_idx)
        ...
        if "rank0_metadata" not in inference_state["tracker_metadata"]:
            inference_state["tracker_metadata"].update(self._initialize_metadata())

        meta = inference_state["tracker_metadata"]
        if obj_id not in meta["obj_id_to_score"]:
            meta["obj_ids_per_gpu"][self.rank] = np.append(meta["obj_ids_per_gpu"][self.rank], obj_id)
            meta["obj_ids_all_gpu"] = np.append(meta["obj_ids_all_gpu"], obj_id)
            meta["num_obj_per_gpu"][self.rank] += 1
            meta["obj_id_to_score"][obj_id] = 1.0
            meta["max_obj_id"] = max(meta["max_obj_id"], obj_id)

            rank0_meta = meta["rank0_metadata"]
            rank0_meta["obj_first_frame_idx"][obj_id] = frame_idx
            if "masklet_confirmation" in rank0_meta:
                mc_status = rank0_meta["masklet_confirmation"]["status"]
                rank0_meta["masklet_confirmation"]["status"] = np.append(mc_status, 0)
                mc_con = rank0_meta["masklet_confirmation"]["consecutive_det_num"]
                rank0_meta["masklet_confirmation"]["consecutive_det_num"] = np.append(mc_con, 0)

        obj_id_to_mask = {obj_id: mask.bool()}
        if self.rank == 0 and inference_state.get("enable_cache", True):
            self._cache_frame_outputs(inference_state, frame_idx, obj_id_to_mask)

        out = {
            "obj_id_to_mask": obj_id_to_mask,
            "obj_id_to_score": {obj_id: 1.0},
            "obj_id_to_tracker_score": {obj_id: 1.0},
        }
        return frame_idx, self._postprocess_output(inference_state, out)
